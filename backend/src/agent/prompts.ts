...
export function buildRegeneratePrompt(
  ...
  return `## Task: Regenerate Cold Email (Using Cached Research)

You already have the research insights from a previous generation. Use these same insights to generate a NEW email variation with a different angle or tone.
...
### Previous Research Insights (USE THESE - DO NOT RESEARCH AGAIN):
- Fit Reasons: ${previousInsights.fit_reasons.join(', ')}
- Company Insights: ${previousInsights.company_insights.join(', ')}
- Unique Angle: ${previousInsights.unique_angle}
- Curiosity Question: ${previousInsights.curiosity_question}

Generate a NEW email that:
- Uses the same insights but presents them differently
- May emphasize different fit reasons
- Could use a slightly different tone (more casual, more direct, etc.)
- Might open with a different hook

Output the same JSON format, keeping the insights identical but with a new email.`;
}
